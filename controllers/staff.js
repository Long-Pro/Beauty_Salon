...
module.exports.addStafff = function (req, res, next) {
  ...
  sql
    ...
    .then((result) => {
      if (result.recordset.length == 0) {
        ...
        res.cookie(
          "errStaff",
          [`Không tồn tại khách hàng có ${filter} là ${value}`],
          {
            maxAge: 1000,
          }
        );
            ...
            sql
              ...
              .then((result) => {
                ...
                async function myDisplay() {
                  ...
                  res.render("staff/viewGuest", {
                    success: [
                      `Lưu thay đổi thông tin khách hàng ${ma} thành công`,
                    ],
                    kq: kq,
                  });
                }
  ...
  sql
    ...
    .then((result) => {
      if (result.recordset[0].ma) {
        ...
        maNext =
          maxMa.slice(0, 2) +
          (parseInt(maxMa.slice(2)) + 1).toString().padStart(8, 0);
      } else {
        maNext = "HD00000001";
      }
      //          insert HOADON
                  ...
                  async function truDiem() {
                    ...
                    sql
                      ...
                      .then((result) => {
                        ...
                        return sql.query`update   khachhang set DIEMTICHLUY=${
                          diem - total
                        } where MA=${maKhach}`;
                      })
                      .then((result) => {
        ...
        sql
          ...
          .then((result) => {
            if (result.recordset.length == 0) {
              // nhan vien k co
              res.cookie("errStaff", [
                `Không tồn tại nhân viên có ${filter} là ${value}`,
              ]);
              res.redirect("/staff/work");
            } else {
              //nhan vien da co
      ...
      if (result.recordset.length == 0) {
        ...
        if (newPassword == confirm_newPassword) {
          sql
            ...
            .then((result) => {
              return sql.query`insert into password values(${md5(
                newPassword
              )})`;
            })
            .then((result) => {
    ...
    .then((result) => {
      ...
      async function myDisplay(hd) {
        ...
        res.render("staff/statistic", {
          ...
          loai: {
            loai: "giua",
            giaTri: `${func.convertTimeYMD(day1)} đến ${func.convertTimeYMD(
              day2
            )}`,
          },
        });
